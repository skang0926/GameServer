...
class LockFreeQueue
{
	...
	struct NodeCounter
	{
		uint32 internalCount : 30; // 30비트만 사용
		uint32 externalCountRemaining : 2; // 2비트만 사용, 합쳐서 32비트 사용
	};
...
public:
	...
	void Push(const T& value)
	{
		...
		while (true)
		{
			// 참조권 획득 (externalCount를 현시점 기준 +1 한 쓰레드
			IncreaseExternalCount(_tail, oldTail);

			// 소유권 획득 (data 먼저 교환 한 쓰레드가 승리)
			T* oldData = nullptr;
	...
	shared_ptr<T> TryPop()
	{
		...
		while (true)
		{
			// 참조권 획득, externalCount를 +1 증가시킨 쓰레드
